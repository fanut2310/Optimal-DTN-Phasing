...
class DTNExpansionOptimizer:
    ...
    def __init__(self, locator: cea.inputlocator.InputLocator, network_type: str,
        ...
        try:
            ...
            # Prices.ELEC_PRICE and NG_PRICE are arrays in USD/Wh; convert to scalar USD/kWh
            ...
            # Fallback defaults if database not available
    ...
    def _calculate_phase_total_expenditure(self, cluster_set, phase):
        ...
        for y in range(phase_duration):
            t = year_offset + (y + 1)  # years from present
            opex_present_value += annual_opex / ((1.0 + self.interest_rate) ** t)

        # Total expenditure is CAPEX (once) plus discounted OPEX
    ...
    def calculate_roi(self, cluster_set, phase):
        ...
        if self.network_type == 'DH':
            annual_demand_kwh = metrics['total_annual_Qh_MWh'] * 1000
        else:
            annual_demand_kwh = metrics['total_annual_Qc_MWh'] * 1000
        ...
        # Apply include_variable_opex gate (already present in original)
        annual_costs = annual_fixed_om + (annual_pump_opex + annual_plant_opex if getattr(self, 'include_variable_opex', True) else 0.0)
        ...
        roi = present_value_of_returns / capex if capex > 0 else 0
        return roi
    ...
    def calculate_district_emissions_new(self):
        ...
        # Load baseline temp demand for possible synthetic generation (fast/hybrid)
        try:
            _baseline_temp_demand_df = pd.read_csv(self.locator.get_dynamic_dtn_optimization_temp_scenario_total_demand())
        except Exception:
            _baseline_temp_demand_df = pd.DataFrame()
        ...
        for phase in range(1, self.num_phases + 1):
            ...
            # Demand cache with mode branching
            if _mode in ('fast', 'hybrid'):
                if phase_demand_cache.exists():
                    log().info(f"[Demand][Synthetic][Cache HIT] Phase {phase} demand (temp): {phase_demand_cache}")
                else:
                    syn = _build_synthetic_phase_demand(phase_supply_df, _baseline_temp_demand_df, original_supply_df)
                    syn.to_csv(phase_demand_cache, index=False)
                    log().info(f"[Demand][Synthetic][Cache MISS] Saved Phase {phase} synthetic demand (temp) to {phase_demand_cache}")
            else:
                if phase_demand_cache.exists():
    ...
    def save_results(self, solution):
        ...
        try:
            ...
            # Also remove an empty 'plots' folder if present
...
class PipeLayoutGenerator:
    ...
    def __init__(self, locator: cea.inputlocator.InputLocator, network_type: str, phase: int = 1, testing_clusters=None,
        ...
        log().info(f"Planned output folder for phase {phase}: {self.output_folder}")
...
def main(config):
    ...
    try:
        ...
        log().info(f"[EmissionsMode] Using mode={optimizer.emissions_mode} (fast-only); hybrid validation disabled.")
    except Exception:
        pass
